...
)
{
  ...
  Fix16 D2 = lawOfCosines(c, Fix16((int16_t)armLength), Fix16((int16_t)forarmLength));
  shoulderAngle = (D1 + D2) * RADIANS_TO_DEGREES;
  elbowAngle = lawOfCosines(Fix16((int16_t)armLength), Fix16((int16_t)forarmLength), c) * RADIANS_TO_DEGREES;
}
